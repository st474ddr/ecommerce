...
class Order extends Model
{
    ...
    {
        return $this->currency == 'NTD' ? '$' : $this->currency;
    }
    ...
    public function statusWithLabel()
    {
        switch ($this->order_status) {
            case 0:
                $result = '<label class="badge badge-success">'.__('New order').'</label>';
                break;
            case 1:
                $result = '<label class="badge badge-warning">'.__('Paid').'</label>';
                break;
            case 2:
                $result = '<label class="badge badge-warning">'.__('Under process').'</label>';
                break;
            case 3:
                $result = '<label class="badge badge-primary">'.__('Finished').'</label>';
                break;
            case 4:
                $result = '<label class="badge badge-danger">'.__('Rejected').'</label>';
                break;
            case 5:
                $result = '<label class="badge badge-dark text-white">'.__('Cancelled').'</label>';
                break;
            case 6:
                $result = '<label class="badge bg-info text-white">'.__('Refund requested').'</label>';
                break;
            case 7:
                $result = '<label class="badge bg-slate">'.__('Returned order').'</label>';
                break;
            case 8:
                $result = '<label class="badge bg-dark text-white">'.__('Refunded').'</label>';
                break;
        }
        return $result;
    }
