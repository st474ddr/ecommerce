...
@section('content')
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex">
            <h6 class="m-0 font-weight-bold text-primary">{{ __('Edit').__('Address') }} ({{ $customer_address->address_title }})</h6>
            <div class="ml-auto">
                <a href="{{ route('admin.customer_addresses.index') }}" class="btn btn-primary">
                    <span class="icon text-white-50">
                        <i class="fa fa-home"></i>
                    </span>
                    <span class="text">{{ __('Customer Addresses') }}</span>
                </a>
            </div>
        </div>
        <div class="card-body">
            ...
            <form action="{{ route('admin.customer_addresses.update', $customer_address->id) }}" method="post">
                ...
                <div class="row">
                    ...
                    <div class="col-3">
                        <div class="form-group">
                            <label for="default_address">{{ __('Default address') }}</label>
                            <select name="default_address" class="form-control">
                                <option value="0" {{ old('default_address', $customer_address->default_address) == '0' ? 'selected' : null }}>{{ __('No') }}</option>
                                <option value="1" {{ old('default_address', $customer_address->default_address) == '1' ? 'selected' : null }}>{{ __('Yes') }}</option>
                            </select>
