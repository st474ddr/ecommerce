...
@section('content')
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex">
            <h6 class="m-0 font-weight-bold text-primary">{{ __('Edit').__('State') }} {{ $state->name }}</h6>
            <div class="ml-auto">
                <a href="{{ route('admin.states.index') }}" class="btn btn-primary">
                    <span class="icon text-white-50">
                        <i class="fa fa-home"></i>
                    </span>
                    <span class="text">{{ __('States') }}</span>
                </a>
            </div>
        </div>
        <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-4">
                        ...
                        <select name="status" class="form-control">
                            <option value="1" {{ old('status', $state->status) == '1' ? 'selected' : null }}>{{ __('Active') }}</option>
                            <option value="0" {{ old('status', $state->status) == '0' ? 'selected' : null }}>{{ __('Inactive') }}</option>
                        </select>
