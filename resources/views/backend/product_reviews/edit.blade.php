...
@section('content')
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex">
            <h6 class="m-0 font-weight-bold text-primary">{{ __('Edit').__('Review') }} ({{ $productReview->product->name }})</h6>
            <div class="ml-auto">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('admin.product_reviews.update', $productReview->id) }}" method="post">
                ...
                <div class="row">
                    ...
                    <div class="col-4">
                        ...
                        <select name="status" class="form-control">
                            <option value="1" {{ old('status', $productReview->status) == '1' ? 'selected' : null }}>{{ __('Active') }}</option>
                            <option value="0" {{ old('status', $productReview->status) == '0' ? 'selected' : null }}>{{ __('Inactive') }}</option>
                        </select>
                ...
                <div class="row">
                    <div class="col-12">
                        <label for="message">{{ __('Review') }}</label>
