...
@section('content')
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex">
            <h6 class="m-0 font-weight-bold text-primary">{{ __('Edit').__('Customer') }} ({{ $customer->full_name }})</h6>
            <div class="ml-auto">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('admin.customers.update', $customer->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="col-3">
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control">
                                <option value="1" {{ old('status', $customer->status) == '1' ? 'selected' : null }}>{{ __('Active') }}</option>
                                <option value="0" {{ old('status', $customer->status) == '0' ? 'selected' : null }}>{{ __('Inactive') }}</option>
                            </select>
                ...
                <div class="row pt-4">
                    <div class="col-12">
                        <label for="cover">{{ __('User Image') }}</label>
                        <br>
                        <div class="file-loading">
                            <input type="file" name="user_image" id="customer-image" class="file-input-overview">
                            <span class="form-text text-muted">{{ __('Image width should be') }} 300px x 300px</span>
